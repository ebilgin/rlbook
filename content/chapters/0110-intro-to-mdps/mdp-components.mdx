...
The set of all possible situations the agent can be in. We denote it $\mathcal{S}$ and individual states as $s \in \mathcal{S}$.
</Definition>
...
The set of choices available to the agent. We denote it $\mathcal{A}$, and the action at time $t$ as $A_t$.
</Definition>
...
<Note>
In some MDPs, available actions depend on the current state. We write $\mathcal{A}(s)$ for the actions available in state $s$. In chess, legal moves depend on the board position.
</Note>
...
<Tip>
A common choice is $\gamma = 0.99$. It's close enough to 1 that the agent cares about the future, but bounded enough for stable learning.
</Tip>
...
|-----------|--------|---------------------|
| States | $\mathcal{S}$ | Where am I? |
| Actions | $\mathcal{A}$ | What can I do? |
| Transitions | $P$ | Where might I end up? |
| Rewards | $R$ | How did that go? |
| Discount | $\gamma$ | How much do I care about the future? |
