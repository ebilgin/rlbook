...
<Definition title="Iterative Policy Evaluation">
  An algorithm that computes $V^\pi$ by repeatedly applying Bellman expectation
  updates to all states until the values converge to the unique fixed point.
</Definition>
...
<Tip>
  In this simple example, information about the reward at C reached state A in
  just two iterations. In general, information propagates one step per
  iteration, so convergence takes roughly as many iterations as the longest path
  to rewards.
</Tip>
...
A gambler has an integer amount of money i, where 0 ≤i≤ N.

Each round of the game:

- With probability p, the gambler **wins** $1 (capital increases by 1).
- With probability q = 1−p, the gambler **loses** $1 (capital decreases by 1).

The game ends when the gambler reaches either absorbing boundary:

- **Ruin:** capital hits 0
- **Goal:** capital hits N

**Question:** What is the Probability of reaching N before 0, starting from i?

**Answer:**
We are going to solve the problem using Bellman equation.

**States:** S=[0,1,2,…,N] where

- 0 = ruined (terminal)
- N = goal reached (terminal)

**Dynamics (for 1≤ i≤ N):**

- go to i + 1 with prob p
- go to i − 1 with prob q = 1 − p

**Step 1:**

Let V(i) = P ⁡(ever hit N ∣ start at i)

So V(i) is the **probability of success**.

**Boundary (terminal) values:**

V(0)=0,V(N)=1

**Step 2:**

From state i, in one step you either go up or down:

V(i)=pV(i+1) + qV(i−1), V(0) = 0, V(N) = 1

**Step 3:**
Now we have two cases ahead of us, if the game is fair, p = q, solution is linear,
the answer would be V(i) = i/N

**Step 4:**

If the game is unbiased, solution has exponential form which is V(i) = A + B (q/p)^i
